...
#----Variáveis reativas para a troca de idiomas do conteúdo do botão Study Area----

selected_language <- reactiveVal("US")  # Inicialmente definido como "US"
...
texts <- reactiveVal(
  list(
    BR = list(
      ...
      scenario_selection = "Escolha um cenário:", # do botão life form modeling
      life_form_selection = "Escolha uma forma de vida:", # do botão life form modeling
      image_slider = "Ajuste o controle deslizante de tempo", # do botão life form modeling
      scenario_selection_species = "Escolha um cenário:", # do botão Especies modeling
      species_selection = "Escolha uma espécie", # do botão Especies modeling
      image_slider_species = "Ajuste o controle deslizante de tempo", # do botão Especies modeling
      scenario_selection_overlay = "Escolha um cenário:", # do botão Overlap maps
      time_interval_slider_overlay = "Ajuste o controle deslizante de tempo" # do botão Overlap maps
    ...
    US = list(
      ...
      scenario_selection = "Choose a scenario:", # do botão life form modeling
      life_form_selection = "Choose a life form:", # do botão life form modeling
      image_slider = "Move the time interval slider:", # do botão life form modeling
      scenario_selection_species = "Choose a scenario:", # do botão Especies modeling
      species_selection = "Choose a species:", # do botão Especies modeling
      image_slider_species = "Move the time interval slider:", # do botão Especies modeling
      scenario_selection_overlay = "Choose a scenario:", # do botão Overlap maps
      time_interval_slider_overlay = "Choose a time interval:" # do botão Overlap maps
...
#----Tela inicial----
  ...
  fluidRow(
    column(4, offset = 4,
           ...
           div(id = "title_div", # Adicione a div com ID "title_div" ao redor do título principal
  ...
  div(
    ...
    shinyjs::hidden(
      div(
        ...
        p(
          HTML("O artigo fonte para este APP foca na Caatinga, uma Floresta Tropical Semiárida (SDTF) no Brasil, 
           cobrindo 833.000 km². A Caatinga se estende por nove estados na região Nordeste e se estende até a parte norte de Minas Gerais no Sudeste. A região frequentemente 
           passa por secas prolongadas com duração de seis a oito meses, com precipitação sendo menos 
           que o dobro da temperatura. O clima é predominantemente semiárido, com uma precipitação anual média de cerca de 800 mm, excedendo 1.000 mm em áreas costeiras, mas caindo abaixo de 300 mm
           em algumas regiões do interior. A estação chuvosa geralmente ocorre de janeiro a maio, embora isso possa variar devido a condições climáticas extremas e diferenças na vegetação pela região. As temperaturas na Caatinga 
           variam ao longo do ano, geralmente variando entre 22ºC e 30ºC.<br/><br/>
           
           Ao selecionar uma forma de vida, você visualiza dentro dos limites do ecossistema da Caatinga no Brasil, as espécies de plantas não nativas e seus pontos georreferenciados coletados da Base Global de Informações sobre Biodiversidade (GBIF) e dos bancos de dados 'Species Link' de 1999 a 2021. 
           Essas ocorrências, juntamente com variáveis de temperatura e precipitação do 'Worldclim.com', foram usadas para modelar a distribuição das espécies em quatro intervalos de tempo: de 2021 a 2040, de 2041 a 2060, de 2061 a 2080 e de 2081 a 2100.<br/><br/>
...
shinyjs::hidden(
  div(
    ...
    # conteúdo da janela de informação em PT-BR
    shinyjs::hidden(
      div(
...
fósseis na economia mundial."),
    ...
    # conteúdo da janela de informação em US
    shinyjs::hidden(
      div(
...
#---- 1: TROCA DO IDIOMA ao CLICAR EM BR e EN na janela INFO ----
...
#----2: TROCA DO IDIOMA ao CLICAR EM BR e US para os botoões principais----
  ...
  # Defina os textos para os botões e títulos da tela inicial
  observe({
    if (selected_language() == "BR") {
      ...
      updateActionButton(session, "species_modeling_button", label = "Modelagem de Espécie 💻")
      updateActionButton(session, "overlap_button", label = "Mapas de Sobreposição 💻")
      
      # títulos em diferentes idiomas
      output$app_title <- renderText("Ecossistema da Caatinga: Espécies Não Nativas nas Décadas Futuras")
      ...
      output$species_modeling_button_text <- renderText("Modelagem de Espécie")
      output$overlap_button_text <- renderText("Mapas de Sobreposição")
    } else {
      # Configuração para o idioma US
      ...
      # títulos em diferentes idiomas
      output$app_title <- renderText("Caatinga Ecosystem: Non-Native Species in the Future Decades")
  ...
  # Mudança de idioma na janela 'Window contents' dento da seção Study Area
  
  
  # Botão 'Window contents' dento da seção Study Area
  ...
  # Fechar janela de informações da seção Study Area (em BR)
  observeEvent(input$close_study_area_info_window_br, {
    shinyjs::hide("study_area_info_window_br")
  })
  
  # Fechar janela de informações da seção Study Area (em US)
  observeEvent(input$close_study_area_info_window_us, {
    shinyjs::hide("study_area_info_window_us")
  })
  
  
  # Mudança de idioma na janela 'Window contents' dento da seção Life form modeling
  
  
  # Botão 'Window contents' dento da seção Life form modeling
  ...
  # Fechar janela de informações da seção Life form modeling (em BR)
  observeEvent(input$close_window_contents_window_br, {
    shinyjs::hide("window_contents_window_br")
  })
  
  # Fechar janela de informações da seção Life form modeling (em US)
  observeEvent(input$close_window_contents_window_us, {
    shinyjs::hide("window_contents_window_us")
  })
  
  # Mudança de idioma na janela 'Window contents' dento da seção Species modeling
  
  # Botão 'Window contents' dento da seção Species modeling
  ...
  # Fechar janela de informações da seção Species modeling (em BR)
  observeEvent(input$close_species_modeling_info_window_br, {
    shinyjs::hide("species_modeling_info_window_br")
  })
  
  # Fechar janela de informações da seção Species modeling (em US)
  observeEvent(input$close_species_modeling_info_window_us, {
    shinyjs::hide("species_modeling_info_window_us")
  })
  
  # Mudança de idioma na janela 'Window contents' dento da seção Overlap maps
  
  # Botão 'Window contents' dento da seção Overlap maps
  ...
  # Fechar janela de informações da seção Overlap maps (em BR)
  observeEvent(input$close_overlap_info_window_br, {
    shinyjs::hide("overlap_info_window_br")
  })
  
  # Fechar janela de informações da seção Overlap maps (em US)
  observeEvent(input$close_overlap_info_window_us, {
    shinyjs::hide("overlap_info_window_us")
  })
  

  # Textos para os botões e títulos em diferentes idiomas da janela Study Area
  observe({
    ...
    updateActionButton(session, "study_area_title", label = texts()[[language]]$caatinga)
  ...
  # Fechar a janela flutuante INFO quando o botão "X" for pressionado
  observeEvent(input$close_info_modal, {
    shinyjs::hide("info_modal")
...
#----BOTÃO 2: Exibir a "nova página" para STUDY AREA quando o botão STUDY AREA for pressionado----
...
#----BOTÃO3: Exibir a "nova página" para LIFE FORMS MODELING quando o botão LIFE FORMS MODELING for pressionado----
  ...
  observeEvent(input$life_forms_button, {
    ...
    observe({
      image_slider_labels <- switch(input$image_slider,
                                    "1" = "*1 = 1st time interval used for modeling (years 2021 to 2040)",
                                    "2" = "",
                                    "3" = "",
                                    "4" = ""
      )
...
#----;BOTÃO 4: Exibir a "nova página" para SPECIES MODELING quando o botão SPECIES MODELING for pressionado----
...
#----BOTÃO 5: Exibir a "nova página" para OVERLAP MAPS quando o botão  OVERLAP MAPS for pressionado----
