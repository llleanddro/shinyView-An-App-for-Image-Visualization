...
# # Load the CSV data from the 'data/species_coordinates' folder.
data <- read.csv("data/species_coordinates/species.csv")

# Load the Caatinga boundaries from the shapefile in the 'data/caatinga_limites' folder.
caatinga_limites <- st_read("data/caatinga_limites/caatinga.shp")

#---- JavaScript to resize the map present in the Study Area button window.----
js_code <- '
$(window).on("resize", function() {
  var width = $(window).width();
  if (width < 768) {
    $("#map").css("height", "200px"); // "Adjust the height for smaller screens."
  } else {
    $("#map").css("height", "365px"); // "Adjust the height for larger screens."
  }
});

$(document).ready(function() {
  $(window).trigger("resize"); // "Enable resizing initially."
});
'

#---- Set custom colors for species in the Species Modeling window ----
...
#---- Action to translate headers and buttons on the initial screen ----
...
#---- Custom Cascading Style Sheets (CSS) for application customization ----
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(
      ...
      /* Style for black bullet icons */
    .bullet-icon {
      color: black; /* Color of the black bullet */
      font-size: 16px; /* Icon size */
      margin-right: 5px; /* Space between icon and species name */
    }

    /* Style for the species-icon class */
    .species-icon {
      display: flex;
      align-items: center;
    }
      /* Size of the header in the Information window */
      .modal-title {
        font-size: 18px;
        font-weight: bold;
        text-align: center;
      ...
      /* Style for smaller screens (max-width: 768px) */
      @media (max-width: 768px) {
        .modal-title {
          font-size: 14px; /* Reduce text size for smaller screens */
        }
      }
      /* Responsive fonts for main buttons and their pop-up windows */
    .responsive-title {
      text-align: center;
      font-size: 1.5vw; /* Using vw (viewport width) for responsive font size */
      font-weight: bold;
      margin-bottom: 9px;
      color: black;
    }
    
    /* Media query to adjust font size of title on smaller screens */
    @media (max-width: 768px) {
      .responsive-title {
      ...
      .modal-content {
        max-width: 50%; /* Set maximum width for the Information pop-up window */
        max-height: 80vh; /* Set maximum height relative to screen height */
        overflow: auto; /* Add scrollbars if content exceeds maximum height */
        position: fixed;
        top: 50%;
        left: 50%;
      ...
      .modal-title {
        font-size: 18px; /* Size of the header in the Information window */
      ...
      /* Add classes and styles for X and Back buttons here */
      .close-button {
        background-color: white;
  ...
  #----Definition of the initial screen----
  
  fluidRow(
    column(4, offset = 4,
           
           div(id = "title_div",
               style = "margin-top: 20px;", # Top spacing
           ...
           # "BR" and "US" buttons for language switch
           div(
  ...
  shinyjs::hidden(
    div(
      ...
      div(
        style = "position: absolute; top: 2px; right: 2px;", # Position of the close button
  ...
  #---- Content of the Study Area Button Window----
  ...
  shinyjs::hidden(
    div(
      ...
      # Header of the Study Area Window
      ...
      # Content of the Information Window (in Portuguese)
    shinyjs::hidden(
      div(
    ...
    # Content of the Information Window (in English)
    shinyjs::hidden(
      div(
  ...
  #---- Content of the Life Forms Modeling Button Window----
...
fósseis na economia mundial."),
    ...
    # Information window content (in US)
    shinyjs::hidden(
      div(
...
#---- Content of the species modeling button window----
shinyjs::hidden(
  div(
      ...
      fluidRow(
        ...
        column(6,
               pickerInput(
                 ...
                 options = list(`actions-box` = TRUE),
                 multiple = FALSE  # This prevents multiple selection
               )
        )
      ),
      
       # Horizontal scrollbar to select the image
      fluidRow(
        column(12,
...
#---- Content of the "new page" for OVERLAP MAPS----
shinyjs::hidden(
  div(
    ...
    # Add the 'Window Contents' button
    div(
      style = "position: absolute; top: 0px; right: 2px;",
...
server <- function(input, output, session) {
  ...
  #----Defining a local directory for Shiny resources URL----
  addResourcePath("www", "www")
  
  #Function to toggle title visibility
  ...
  observeEvent(input$info_button, {
    shinyjs::hide("title_div") # Making the title invisible
  ...
  # Reactive object to store the selected language
  selected_language <- reactiveVal("US")
  
  # Update selected language based on BR and US buttons
...
# Render the content of the Information button window
  ...
  # Definition of texts for the buttons on the home screen and the title
  observe({
  ...
  # Changing language of window headers for buttons on the home screen
  
  # Life Form Modeling Button
  observe({
  ...
  # Language change in the 'Window Content' within the Study Area section
  ...
  # Language change in the 'Window Content' window within the Life Form Modeling section
  ...
  # Language change in the 'Window Content' window within the Species Modeling section
  ...
  # Language change in the 'Window Content' section within the Overlay Maps
  ...
  # Texts for buttons and titles in different languages of the Study Area window
  observe({
    language <- selected_language()
    ...
    # Update the text using renderText
  ...
  # Close the floating window and make the title visible when the "X" button is pressed
  observeEvent(input$close_info_modal, {
    shinyjs::show("title_div") # Making the title visible
    shinyjs::hide("info_modal")
  })
  
  # Display the floating window when the Information button is pressed
  observeEvent(input$info_button, {
    shinyjs::show("info_modal")
  ...
  # Close the floating window "Information" when the "X" button is pressed
  observeEvent(input$close_info_modal, {
    shinyjs::hide("info_modal")
...
#----Settings to display the window when the 'Study Area' button is clicked----
  observeEvent(input$study_area_button, {
    toggleTitleVisibility(FALSE) # Making the title visible
    shinyjs::hide("info_button")
    shinyjs::hide("study_area_button")
    shinyjs::hide("life_forms_button")
    shinyjs::hide("species_modeling_button")
  ...
  # Return to the home page when the "Back" button is pressed on the STUDY AREA page
...
#----Settings to display the window when the 'Life Form Modeling' button is clicked----
  
  observeEvent(input$life_forms_button, {
    toggleTitleVisibility(FALSE) # Tornar o título visível
    shinyjs::hide("info_button")
    shinyjs::hide("study_area_button")
    shinyjs::hide("life_forms_button")
    shinyjs::hide("species_modeling_button")
  ...
  # Return to the home page when the "Back" button is pressed on the LIFE FORMS MODELING page
  observeEvent(input$back_to_main2, {
    toggleTitleVisibility(TRUE) # Making the title visible
    shinyjs::show("info_button")
    shinyjs::show("study_area_button")
    shinyjs::show("life_forms_button")
    shinyjs::show("species_modeling_button")
...
#----Settings to display the window when the 'Species Modeling' button is clicked----
  ...
  observeEvent(input$species_modeling_button, {
    ...
    updateSliderInput(session, "image_slider_species", value = 1) # Restart the slider
  })
  ...
  # Return to the home page when the "Back" button is pressed on the SPECIES MODELING page
  ...
  #----Settings to display the window when the 'Overlap Maps' button is clicked----
  
  observeEvent(input$overlap_button, {
    toggleTitleVisibility(FALSE) # Make the title visible
    shinyjs::hide("info_button")
    shinyjs::hide("study_area_button")
    shinyjs::hide("life_forms_button")
    shinyjs::hide("species_modeling_button")
  ...
  #----Map and geo occurrences settings within the 'Study Area' button window----
  ...
  # Function to add markers on the map with different colors based on the species
  observe({
    req(input$life_form_selector)  # Ensure that a life form has been selected
    ...
    # Filter the data based on the selected life form
    ...
    # Add markers with different colors based on species
    leafletProxy("map") %>%
      clearMarkers() %>%
      addCircleMarkers(data = markers_df, 
                       lng = ~longitude, lat = ~latitude,
                       radius = 2,  # Size of the circles (adjust as needed)
                       fillOpacity = 1,  # Fill opacity (adjust as needed)
                       color = ~color,
                       stroke = TRUE,
                       weight = 1,
                       group = "species_markers",  # Add a group to control the markers
                       popup = ~species)
  })
  
  # Generate the species list based on the selected life form.
          ...
          tagList(
            div(
              ...
              HTML(paste0('<span class="bullet-icon" style="color:', 
                          species_colors[species_name],  # Use of the colors defined earlier
                          '">&#9679;</span>')),
              species_name
            )
          )
  ...
  #----Render the image based on the scenario, LIFE FORM, and slider position----
  
  output$life_forms_image <- renderUI({
    req(input$scenario_selection, input$life_form_selection, input$image_slider) # Check if all selections are made
    
    scenario <- input$scenario_selection
    life_form <- input$life_form_selection
    image_number <- input$image_slider
    
    # Construct the image path based on life form, scenario, and image number
...
#----Render the image based on the scenario, the SPECIES, and the slider position.----
  
  output$species_modeling_image <- renderUI({
    req(input$scenario_selection_species, input$species_selection, input$image_slider_species) #Checks if all selections have been made.
    ...
    if (species == "Prosopis juliflora") {
      ...
      # Otherwise, does not display image
      div()
    }
  ...
  #----Render the image based on the scenario and slider position----
  output$overlay_image <- renderUI({
    req(input$scenario_selection_overlay, input$time_interval_slider_overlay) # Check selections
    scenario <- input$scenario_selection_overlay
    image_number <- input$time_interval_slider_overlay
    
    # Construct the image path based on the scenario and slider position
